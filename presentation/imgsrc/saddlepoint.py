...
c = canvas.canvas()
p = painter.regular(basepathattrs=[deco.earrow], titlepos=1,
                    outerticklength=painter.ticklength.normal,
                    titledist=-0.35, titleattrs=[text.halign.right], titledirection=None) # horizontal text

p1y = painter.regular(basepathattrs=[deco.earrow], titlepos=1.03,
                    outerticklength=painter.ticklength.normal,
                    titledist=0.15, titleattrs=[text.valign.top], titledirection=None) # horizontal text

g1 = c.insert(
         graphxy(width=5, height=2, xaxisat=0, yaxisat=0,
             x=axis.linear(title="$x$", min=0, max=xrange,
                           painter=p, parter=None),
             y=axis.linear(title=r"$I(x)$", max=1.2, painter=p1y, parter=None)
         )
     )
...
g1.plot(data.points(list(zip(np.sqrt(x2), integral)), x=1, y=2), lineproperties)

g2 = c.insert(
         graphxy(xpos=g1.xpos-g1.width, ypos=g1.ypos+g1.height+0.5,
             width=10, height=2, xaxisat=0, yaxisat=0,
             x=axis.linear(title="$x$", min=-xrange, max=xrange, painter=p,
                           parter=None),
             y=axis.linear(title=r"$f(x)$", min=-1.1, max=2, painter=p1y,
                           parter=None)
         )
     )
...
phase = np.cos(x2)
for sign in (-1, 1):
    g2.plot(data.points(list(zip(sign*np.sqrt(x2), phase)), x=1, y=2), lineproperties)
...
g3 = c.insert(
         graphxy(xpos=g2.xpos, ypos=g2.ypos+g2.height+0.5,
             ...
             x=axis.linear(title="$x$", min=-xrange, max=xrange, painter=p,
                           parter=None),
             y=axis.linear(title=r"$S(x)$", painter=p1y, parter=None)
         )
     )
